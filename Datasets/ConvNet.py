...
def create_reader(map_file, is_training):
    ...
    # transformation pipeline for the features has jitter/crop only when
    # training
    transforms = []
    if is_training:
        transforms += [
...
def LocalResponseNormalization(k, n, alpha, beta, name=''):
    ...
    x2 = cntk.square(x)
    # reshape to insert a fake singleton reduction dimension after the 3th
    # axis (channel axis). Note Python axis order and BrainScript are
    # reversed.
    x2s = cntk.reshape(x2, (1, cntk.InferredDimension), 0, 1)
    W = cntk.constant(alpha / (2 * n + 1), (1, 2 * n + 1, 1, 1), name='W')
    # 3D convolution with a filter that has a non 1-size only in the 3rd axis,
    # and does not reduce since the reduction dimension is fake and 1
...
def convnetlrn_cifar10_dataaug(reader_train, reader_test, epoch_size=50000, max_epochs=80):
    ...
    lr_per_sample = [0.0015625] * 20 + [0.00046875] * 20 + \
        [0.00015625] * 20 + [0.000046875] * 10 + [0.000015625]
    lr_schedule = cntk.learning_rate_schedule(
        lr_per_sample, unit=cntk.learners.UnitType.sample, epoch_size=epoch_size)
    ...
    while sample_count < epoch_size:
        ...
        data = reader_test.next_minibatch(
            current_minibatch, input_map=input_map)
    ...
    print("Final Results: Minibatch[1-{}]: errs = {:0.2f}% * {}".format(
        minibatch_index + 1, (metric_numer * 100.0) / metric_denom, metric_denom))
    print("")
...
if __name__ == '__main__':
    reader_train = create_reader(os.path.join(
        data_path, 'train_map.txt'), True)
    reader_test = create_reader(os.path.join(
        data_path, 'test_map.txt'), False)
